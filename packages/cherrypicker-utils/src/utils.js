...
async function loadFiles(paths) {
  let filePaths = await reduce(
    paths,
    async (argFilePaths, arg) => argFilePaths.concat(await glob(arg)),
    []
  );
  filePaths = await filter(
    filePaths,
    async argFile => !(await stats(argFile)).isDirectory()
  );
